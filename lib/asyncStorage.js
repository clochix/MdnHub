...
(function () {
  ...
  function getAll(callback, transform) {

    var res = {},
        doTransform = (typeof transform === 'function');
    ...
    withStore('readonly', function keyBody(store) {
      ...
      req.onsuccess = function keyOnSuccess() {
        ...
        if (!cursor) {
          ...
          res[cursor.key] = doTransform ? transform(cursor.value) : cursor.value;
          cursor.continue();
        }
      };
      req.onerror = function keyOnError() {
        console.error('Error in asyncStorage.getAll(): ', req.error.name);
      };
    });
  }
